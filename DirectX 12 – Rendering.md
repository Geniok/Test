...
-   [6  The Command Queue Class](https://www.3dgep.com/learning-directx12-2/#The_Command_Queue_Class)
    -   [6.1  CommandQueue Header](https://www.3dgep.com/learning-directx12-2/#CommandQueue_Header)
        -   [6.1.1  CommandQueue Preamble](https://www.3dgep.com/learning-directx12-2/#CommandQueue_Preamble)
...
-   [9  The Tutorial Class](https://www.3dgep.com/learning-directx12-2/#The_Tutorial_Class)
    ...
    -   [9.12  Tutorial2::OnRender](https://www.3dgep.com/learning-directx12-2/#Tutorial2OnRender)
        ...
        -   [9.12.5  Bind the Render Targets](https://www.3dgep.com/learning-directx12-2/#Bind_the_Render_Targets)
...
Similar to the  `CommandAllocatorEntry`, the  `CommandListQueue`  alias describes an  `[std::queue](http://en.cppreference.com/w/cpp/container/queue)`  object that is used to queue command lists that can be reused. There is no need to associate a fence value with the command lists since they can be reused right after they have been executed on the command queue.
...
I didn’t show the implementation details of the application class, but this class can be viewed on GitHub:  [https://git.io/vApW1](https://git.io/vApW1)  
...
The  `[ID3D12Device::CreateCommittedResource](https://msdn.microsoft.com/en-us/library/dn899178(v=vs.85).aspx)`  method is used to create a resource and an implicit heap that is large enough to store the resource. The resource is also mapped to the implicit heap.

The  `[ID3D12Device::CreateCommittedResource](https://msdn.microsoft.com/en-us/library/dn899178(v=vs.85).aspx)`  has the following signature  [[9]](https://www.3dgep.com/learning-directx12-2/#cite-9):
...
-   `[D3D12_HEAP_PROPERTIES](https://msdn.microsoft.com/en-us/library/dn770373(v=vs.85).aspx) *pHeapProperties`: A pointer to a  `[D3D12_HEAP_PROPERTIES](https://msdn.microsoft.com/en-us/library/dn770373(v=vs.85).aspx)`structure that provides properties for the resource’s heap.
-   `[D3D12_HEAP_FLAGS](https://msdn.microsoft.com/en-us/library/dn986730(v=vs.85).aspx) HeapFlags`: Heap options, as a bitwise-OR’d combination of  `[D3D12_HEAP_FLAGS](https://msdn.microsoft.com/en-us/library/dn986730(v=vs.85).aspx)`  enumeration constants.
-   `[D3D12_RESOURCE_DESC](https://msdn.microsoft.com/en-us/library/dn903813(v=vs.85).aspx) *pDesc`: A pointer to a  `[D3D12_RESOURCE_DESC](https://msdn.microsoft.com/en-us/library/dn903813(v=vs.85).aspx)`  structure that describes the resource.
-   `[D3D12_RESOURCE_STATES](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx) InitialResourceState`: The initial state of the resource, as a bitwise-OR’d combination of  `[D3D12_RESOURCE_STATES](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx)`  enumeration constants.  
    When a resource is created together with a  `[D3D12_HEAP_TYPE_UPLOAD](https://msdn.microsoft.com/en-us/library/dn770374(v=vs.85).aspx#D3D12_HEAP_TYPE_UPLOAD)`  heap,  `InitialResourceState`  must be  `[D3D12_RESOURCE_STATE_GENERIC_READ](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx#D3D12_RESOURCE_STATE_GENERIC_READ)`. When a resource is created together with a  `[D3D12_HEAP_TYPE_READBACK](https://msdn.microsoft.com/en-us/library/dn770374(v=vs.85).aspx#D3D12_HEAP_TYPE_READBACK)`  heap,  `InitialResourceState`  must be  `[D3D12_RESOURCE_STATE_COPY_DEST](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx#D3D12_RESOURCE_STATE_COPY_DEST)`.
-   `[D3D12_CLEAR_VALUE](https://msdn.microsoft.com/en-us/library/dn903795(v=vs.85).aspx) *pOptimizedClearValue`: Specifies a  `[D3D12_CLEAR_VALUE](https://msdn.microsoft.com/en-us/library/dn903795(v=vs.85).aspx)`  that describes the default value for a clear color.  
    `pOptimizedClearValue`  specifies a value for which clear operations are most optimal. When the created resource is a texture with either the  `[D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET](https://msdn.microsoft.com/en-us/library/dn986742(v=vs.85).aspx#D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET)`  or  `[D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL](https://msdn.microsoft.com/en-us/library/dn986742(v=vs.85).aspx#D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)`  flags, applications should choose the value that the clear operation will most commonly be called with. Clear operations can be called with other values, but those operations will not be as efficient as when the value matches the one passed into resource creation.  `pOptimizedClearValue`  must be  `NULL`  when used with  `[D3D12_RESOURCE_DIMENSION_BUFFER](https://msdn.microsoft.com/en-us/library/dn770396(v=vs.85).aspx#D3D12_RESOURCE_DIMENSION_BUFFER)`.
-   `REFIID riidResource`: The globally unique identifier (GUID) for the resource interface. This is an input parameter. The  `REFIID`, or GUID, of the interface to the resource can be obtained by using the  `[__uuidof](https://docs.microsoft.com/en-us/cpp/cpp/uuidof-operator?view=vs-2017)()`  macro. For example,  `[__uuidof](https://docs.microsoft.com/en-us/cpp/cpp/uuidof-operator?view=vs-2017)([ID3D12Resource](https://msdn.microsoft.com/en-us/library/dn788709(v=vs.85).aspx))`  will get the GUID of the interface to a resource.
...
If the buffer data isn’t  `NULL`  then another committed resource is created using the  `[ID3D12Device::CreateCommittedResource](https://msdn.microsoft.com/en-us/library/dn899178(v=vs.85).aspx)`  method. In this case, the heap type is set to  `[D3D12_HEAP_TYPE_UPLOAD](https://msdn.microsoft.com/en-us/library/dn770374(v=vs.85).aspx#D3D12_HEAP_TYPE_UPLOAD)`  and the resource state is initailly set to  `[D3D12_RESOURCE_STATE_GENERIC_READ](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx#D3D12_RESOURCE_STATE_GENERIC_READ)`.

Resources created in a  `[D3D12_HEAP_TYPE_UPLOAD](https://msdn.microsoft.com/en-us/library/dn770374(v=vs.85).aspx#D3D12_HEAP_TYPE_UPLOAD)`  heap must be created with  `[D3D12_RESOURCE_STATE_GENERIC_READ](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx#D3D12_RESOURCE_STATE_GENERIC_READ)`  and cannot be changed away from this.  
...
The  `[D3D12_SUBRESOURCE_DATA](https://msdn.microsoft.com/en-us/library/dn879485(v=vs.85).aspx)`  structure is used to describe the subresource data that is to be uploaded to the GPU resource.

This structure has the following syntax  [[10]](https://www.3dgep.com/learning-directx12-2/#cite-10):
...
On line 106-109, the  `[UpdateSubresources](https://msdn.microsoft.com/en-us/library/dn899213(v=vs.85).aspx)`  function is used to upload the CPU buffer data to the GPU resource in a default heap using an intermediate buffer in an upload heap.

The  `[UpdateSubresources](https://msdn.microsoft.com/en-us/library/dn899213(v=vs.85).aspx)`  method has the following signature  [[11]](https://www.3dgep.com/learning-directx12-2/#cite-11):
...
-   `[ID3D12GraphicsCommandList](https://msdn.microsoft.com/en-us/library/dn903537(v=vs.85).aspx) *pCmdList`: A pointer to the  `[ID3D12GraphicsCommandList](https://msdn.microsoft.com/en-us/library/dn903537(v=vs.85).aspx)`  interface for the command list.
-   `[ID3D12Resource](https://msdn.microsoft.com/en-us/library/dn788709(v=vs.85).aspx) *pDestinationResource`: A pointer to the  `[ID3D12Resource](https://msdn.microsoft.com/en-us/library/dn788709(v=vs.85).aspx)`interface that represents the destination resource.
-   `[ID3D12Resource](https://msdn.microsoft.com/en-us/library/dn788709(v=vs.85).aspx) *pIntermediate`: A pointer to the  `[ID3D12Resource](https://msdn.microsoft.com/en-us/library/dn788709(v=vs.85).aspx)`  interface that represents the intermediate resource.
...
-   `[D3D12_SUBRESOURCE_DATA](https://msdn.microsoft.com/en-us/library/dn879485(v=vs.85).aspx) *pSrcData`: Pointer to an array (of length  `NumSubresources`) of pointers to  `[D3D12_SUBRESOURCE_DATA](https://msdn.microsoft.com/en-us/library/dn879485(v=vs.85).aspx)`  structures containing descriptions of the subresource data used for the update.
...
The vertex buffer data and index buffer data require a  `[ID3D12GraphicsCommandList](https://msdn.microsoft.com/en-us/library/dn903537(v=vs.85).aspx)`  to perform the copy operation. Creating and retrieving command lists was described in the  [Command Queue Class](https://www.3dgep.com/learning-directx12-2/#The_Command_Queue_Class)  section above. The  `LoadContent`  function uses the (copy)  `CommandQueue`  class to retrieve a command list that can be used to upload the buffer data to the GPU resources.
...
On line 116 the  `[D3D12_COMMAND_LIST_TYPE_COPY](https://msdn.microsoft.com/en-us/library/dn770348(v=vs.85).aspx#D3D12_COMMAND_LIST_TYPE_COPY)`  command queue is retrieved from the  `Application`  instance. On line 117 a  `[ID3D12GraphicsCommandList](https://msdn.microsoft.com/en-us/library/dn903537(v=vs.85).aspx)`  is retrieved from the  `[CommandQueue](https://www.3dgep.com/learning-directx12-2/#The_Command_Queue_Class)`.

Next, the buffer data is copied to the GPU resources.

### UPLOAD VERTEX BUFFER

The  `[Tutorial2::UpdateBufferResource](https://www.3dgep.com/learning-directx12-2/#Tutorial2::UpdateBufferResource)`  method described earlier is used to create the GPU resources and upload the vertex buffer data to the GPU.
...
The  `m_VertexBufferView`  member variable is the  `[D3D12_VERTEX_BUFFER_VIEW](https://msdn.microsoft.com/en-us/library/dn903819(v=vs.85).aspx)`  structure that describes the vertex buffer. The  `[D3D12_VERTEX_BUFFER_VIEW](https://msdn.microsoft.com/en-us/library/dn903819(v=vs.85).aspx)`  structure has the following syntax  [[12]](https://www.3dgep.com/learning-directx12-2/#cite-12):
...
Similar to the vertex buffer data, the  `[Tutorial2::UpdateBufferResource](https://www.3dgep.com/learning-directx12-2/#Tutorial2::UpdateBufferResource)`  method is used to create the GPU resources and upload the index buffer data to the GPU.
...
Similar to the  [Vertex Buffer View](https://www.3dgep.com/learning-directx12-2/#Vertex_Buffer_View), the  **Index Buffer View**  is used to describe the index buffer to the  **Input Assembler**  stage of the rendering pipeline.
...
The  `m_IndexBufferView`  member variable is the  `[D3D12_INDEX_BUFFER_VIEW](https://msdn.microsoft.com/en-us/library/dn891445(v=vs.85).aspx)`  structure that is used to describe the index buffer.

The  `[D3D12_INDEX_BUFFER_VIEW](https://msdn.microsoft.com/en-us/library/dn891445(v=vs.85).aspx)`  structure has the following syntax  [[13]](https://www.3dgep.com/learning-directx12-2/#cite-13):
...
-   `[DXGI_FORMAT](https://msdn.microsoft.com/en-us/library/bb173059(v=vs.85).aspx) Format`: A  `[DXGI_FORMAT](https://msdn.microsoft.com/en-us/library/bb173059(v=vs.85).aspx)`-typed value for the index-buffer format. In this case, the  `[DXGI_FORMAT_R16_UINT](https://msdn.microsoft.com/en-us/library/bb173059(v=vs.85).aspx#DXGI_FORMAT_R16_UINT)`  enumeration value is used to indicate that each value in the index buffer is a single component 16-bit unsigned integer.

### DSV DESCRIPTOR HEAP

In the  [previous lesson](https://www.3dgep.com/learning-directx12-1/), a swap-chain is created to be the target for the final rendered image that is presented to the user’s screen. The swap-chain can contain two or more texture buffers but it does not create a  **depth buffer**. A  **depth buffer**  is not strictly required for rendering. When all of the elements in a scene are 2D (common for side-scrolling platform games), then the scene elements can be rendered from back-to-front and all elements will appear correctly. When rendering a 3D scene (even if the scene only contains a single geometric object) then a  **depth buffer**  is required for achieving correct rendering. The  **depth buffer**  stores the depth of a pixel in  **normalized device coordinate space**  (NDC). Each pixel stores a depth value in the range  {0⋯1}{0⋯1}  where  00  is closest to the viewer (near clipping plane) and  11  is farthest away (far clipping plane).

[![Depth Buffer](https://www.3dgep.com/wp-content/uploads/2018/03/Depth-Buffer-View-Space-Normalized-Device-Coordinate-Space.png)](https://www.3dgep.com/wp-content/uploads/2018/03/Depth-Buffer-View-Space-Normalized-Device-Coordinate-Space.png)
...
The  `[D3D12_DESCRIPTOR_HEAP_DESC](https://msdn.microsoft.com/en-us/library/dn770359(v=vs.85).aspx)`  structure is used to describe the descriptor heap.

The  `[D3D12_DESCRIPTOR_HEAP_DESC](https://msdn.microsoft.com/en-us/library/dn770359(v=vs.85).aspx)`  structure has the following syntax:
...
The descriptor heap is created on line 146 using the  `[ID3D12Device::CreateDescriptorHeap](https://msdn.microsoft.com/en-us/library/dn788662(v=vs.85).aspx)`  method.

### LOAD THE SHADERS

If the Visual Studio project is configured correctly, then any HLSL shader file (any file with the  **.hlsl**  extension) that is added to the project will automatically be compiled into a shader object file that can be loaded by the application at runtime. By default, shaders are compiled into a  **Compiled Shader Object**  file (**.cso**) with the same name as the original .hlsl file. For example,  `VertexShader.hlsl`  will be compiled as  `VertexShader.cso`. The CSO file can be loaded directly into a binary object using the  `[D3DReadFileToBlob](https://msdn.microsoft.com/en-us/library/hh446877(v=vs.85).aspx)`  function.

When using the  `[D3DReadFileToBlob](https://msdn.microsoft.com/en-us/library/hh446877(v=vs.85).aspx)`  function, or any function in the  `[D3DCompiler](https://msdn.microsoft.com/en-us/library/dd607340(v=vs.85).aspx)`  API, do not forget to include the  `[d3dcompiler.h](https://msdn.microsoft.com/en-us/library/dd607340(v=vs.85).aspx)`  header file in your source and link against the  `D3Dcompiler_47.lib`  library and copy the  `D3dcompiler_47.dll`  file to the same folder as the binary executable when distributing your project.
...
The  `[D3DReadFileToBlob](https://msdn.microsoft.com/en-us/library/hh446877(v=vs.85).aspx)`  function takes the path to the file (as a wide character string) to load and returns a pointer to a  `[ID3DBlob](https://msdn.microsoft.com/en-us/library/ff728743(v=vs.85).aspx)`  object that stores the compiled shader in binary format.
...
The  **Input Layout**  is specified using an array of  `[D3D12_INPUT_ELEMENT_DESC](https://msdn.microsoft.com/en-us/library/dn770377(v=vs.85).aspx)`  structures. This structure has the following syntax  [[14]](https://www.3dgep.com/learning-directx12-2/#cite-14):
...
-   `[DXGI_FORMAT](https://msdn.microsoft.com/en-us/library/bb173059(v=vs.85).aspx) Format`: A  `[DXGI_FORMAT](https://msdn.microsoft.com/en-us/library/bb173059(v=vs.85).aspx)`-typed value that specifies the format of the element data.
...
-   `[D3D12_INPUT_CLASSIFICATION](https://msdn.microsoft.com/en-us/library/dn770376(v=vs.85).aspx) InputSlotClass`: A value that identifies the input data class for a single input slot. Valid values for  **InputSlotClass**  are:
    -   `[D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA](https://msdn.microsoft.com/en-us/library/dn770376(v=vs.85).aspx#D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA)`: Input data is per-vertex data.
    -   `[D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA](https://msdn.microsoft.com/en-us/library/dn770376(v=vs.85).aspx#D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA)`: Input data is per-instance data.
-   `UINT InstanceDataStepRate`: The number of instances to draw using the same per-instance data before advancing in the buffer by one element. This value must be 0 for an element that contains per-vertex data (the slot class is set to  `[D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA](https://msdn.microsoft.com/en-us/library/dn770376(v=vs.85).aspx#D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA)`).
...
The first per-vertex attribute is bound to the  `POSITION`  semantic at index 0 in the HLSL shader. The data contains a 3-component floating-point format (`[DXGI_FORMAT_R32G32B32_FLOAT](https://msdn.microsoft.com/en-us/library/bb173059(v=vs.85).aspx#DXGI_FORMAT_R32G32B32_FLOAT)`) and the buffer is bound to input slot 0 of the  **Input Assembler**. The offset to the first element in the buffer is 0 and the elements are automatically aligned using the  `D3D12_APPEND_ALIGNED_ELEMENT`  macro.
...
**Exercise**  
It is possible to avoid the use of  **Input Layouts**  by storing the vertex buffer data in one or more  `[StructuredBuffer](https://msdn.microsoft.com/en-us/library/ff471514(v=vs.85).aspx)`s in HLSL. Try changing the vertex shader in this demo to read from a  `[StructuredBuffer](https://msdn.microsoft.com/en-us/library/ff471514(v=vs.85).aspx)`  in HLSL instead of using a vertex buffer.  

### ROOT SIGNATURE

The root signature describes the parameters that are passed to the programmable stages of the rendering (or compute) pipeline. Before the pipeline shader object (PSO) can be created, the root signature needs to be defined. The various types of root signature parameters were described in the previous section titled  [Root Signature Parameters](https://www.3dgep.com/learning-directx12-2/#Root_Signature_Parameters). In this section, the source code to configure a simple root signature is shown.
...
Root signature version 1.1 is preferred but the driver supported version on the end-user’s computer must be verified using the  `[ID3D12Device::CheckFeatureSupport](https://msdn.microsoft.com/en-us/library/dn788653(v=vs.85).aspx)`method. If the check fails, the root signature version will fall-back to the root signature version 1.0.
...
The supported root signature flags are  [[16]](https://www.3dgep.com/learning-directx12-2/#cite-16):

-   `[D3D12_ROOT_SIGNATURE_FLAG_NONE](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx#D3D12_ROOT_SIGNATURE_FLAG_NONE)`: The default behaviour.
-   `[D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx#D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT)`: The app is opting in to using the Input Assembler (requiring an input layout that defines a set of vertex buffer bindings). Omitting this flag can result in one root argument space being saved on some hardware. Omit this flag if the Input Assembler is not required, though the optimization is minor.
-   `[D3D12_ROOT_SIGNATURE_FLAG_DENY_VERTEX_SHADER_ROOT_ACCESS](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx#D3D12_ROOT_SIGNATURE_FLAG_DENY_VERTEX_SHADER_ROOT_ACCESS)`: Denies the vertex shader access to the root signature.
-   `[D3D12_ROOT_SIGNATURE_FLAG_DENY_HULL_SHADER_ROOT_ACCESS](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx#D3D12_ROOT_SIGNATURE_FLAG_DENY_HULL_SHADER_ROOT_ACCESS)`: Denies the hull shader access to the root signature.
-   `[D3D12_ROOT_SIGNATURE_FLAG_DENY_DOMAIN_SHADER_ROOT_ACCESS](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx#D3D12_ROOT_SIGNATURE_FLAG_DENY_DOMAIN_SHADER_ROOT_ACCESS)`: Denies the domain shader access to the root signature.
-   `[D3D12_ROOT_SIGNATURE_FLAG_DENY_GEOMETRY_SHADER_ROOT_ACCESS](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx#D3D12_ROOT_SIGNATURE_FLAG_DENY_GEOMETRY_SHADER_ROOT_ACCESS)`: Denies the geometry shader access to the root signature.
-   `[D3D12_ROOT_SIGNATURE_FLAG_DENY_PIXEL_SHADER_ROOT_ACCESS](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx#D3D12_ROOT_SIGNATURE_FLAG_DENY_PIXEL_SHADER_ROOT_ACCESS)`: Denies the pixel shader access to the root signature.
-   `[D3D12_ROOT_SIGNATURE_FLAG_ALLOW_STREAM_OUTPUT](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx#D3D12_ROOT_SIGNATURE_FLAG_ALLOW_STREAM_OUTPUT)`: The root signature allows stream output. This flag can be specified for root signatures authored in HLSL, similar to how the other flags are specified.  `[ID3D12Device::CreateGraphicsPipelineState](https://msdn.microsoft.com/en-us/library/dn788663(v=vs.85).aspx)`  will fail if the geometry shader contains stream output but the root signature does not have this flag set. Omit this flag if stream output is not required.
...
The  `[CD3DX12_ROOT_PARAMETER1](https://msdn.microsoft.com/en-us/library/mt750341(v=vs.85).aspx)`  is a helper structure from the  `d3dx12.h`  header file that allows for easy initialization of a  `[D3D12_ROOT_PARAMETER1](https://msdn.microsoft.com/en-us/library/mt709123(v=vs.85).aspx)`  structure. The  `InitAsConstants`  method takes the following parameters:
...
-   `[D3D12_SHADER_VISIBILITY](https://msdn.microsoft.com/en-us/library/dn879482(v=vs.85).aspx) visibility`: Specifies the shader stages that are allowed to access the contents at that root signature slot. In this case, the visibility of the 32-bit constants is restricted to the vertex shader stage.
...
The  `[CD3DX12_VERSIONED_ROOT_SIGNATURE_DESC](https://msdn.microsoft.com/en-us/library/mt750342(v=vs.85).aspx)`  is a helper structure from the  **d3dx12.h**header file that allows for easy initialization of a  `[D3D12_VERSIONED_ROOT_SIGNATURE_DESC](https://msdn.microsoft.com/en-us/library/mt709125(v=vs.85).aspx)`structure.
...
-   `[D3D12_ROOT_PARAMETER1](https://msdn.microsoft.com/en-us/library/mt709123(v=vs.85).aspx)* _pParameters`: An array of  **numParameters**  root parameters. The array of root parameters contains only a single 32-bit root constant parameter.
-   `UINT numStaticSamplers`: The number of static samplers in the root signature.
-   `[D3D12_STATIC_SAMPLER_DESC](https://msdn.microsoft.com/en-us/library/dn986748(v=vs.85).aspx)* _pStaticSamplers`: An array of  **numStaticSamplers**static samplers.
-   `[D3D12_ROOT_SIGNATURE_FLAGS](https://msdn.microsoft.com/en-us/library/dn879480(v=vs.85).aspx) flags`: Flags that determine the root signature visibility to the various shader stages.
...
On line 191 the actual root signature object is created from the serialized root signature using the  `[ID3D12Device::CreateRootSignature](https://msdn.microsoft.com/en-us/library/dn899182(v=vs.85).aspx)`  method.
...
[https://github.com/Microsoft/DirectX-Graphics-Samples/tree/master/Libraries/D3DX12](https://github.com/Microsoft/DirectX-Graphics-Samples/tree/master/Libraries/D3DX12)  
...
-   `[CD3DX12_PIPELINE_STATE_STREAM_FLAGS](https://msdn.microsoft.com/en-us/library/mt800526(v=vs.85).aspx)`: Wrapper for a  `[D3D12_PIPELINE_STATE_FLAGS](https://msdn.microsoft.com/en-us/library/dn986737(v=vs.85).aspx)`  enumeration.
-   `[CD3DX12_PIPELINE_STATE_STREAM_NODE_MASK](https://msdn.microsoft.com/en-us/library/mt800531(v=vs.85).aspx)`: Describes the pipeline state node mask as a UINT, which is used to identify the nodes (physical adapters of the device) that the PSO applies to in Multi-Adapter scenarios; each bit in the mask corresponds to a single node. For single-adapter scenarios, set this value to 0.
-   `[CD3DX12_PIPELINE_STATE_STREAM_ROOT_SIGNATURE](https://msdn.microsoft.com/en-us/library/mt800536(v=vs.85).aspx)`: Wrapper for a pointer to a  `[ID3D12RootSignature](https://msdn.microsoft.com/en-us/library/dn788714(v=vs.85).aspx)`.
-   `[CD3DX12_PIPELINE_STATE_STREAM_INPUT_LAYOUT](https://msdn.microsoft.com/en-us/library/mt800530(v=vs.85).aspx)`: Wrapper for a  `[D3D12_INPUT_LAYOUT_DESC](https://msdn.microsoft.com/en-us/library/dn770378(v=vs.85).aspx)`. Specifies the input layout for the input assembly stage.
-   `[CD3DX12_PIPELINE_STATE_STREAM_IB_STRIP_CUT_VALUE](https://msdn.microsoft.com/en-us/library/mt800529(v=vs.85).aspx)`: Wrapper for a  `[D3D12_INDEX_BUFFER_STRIP_CUT_VALUE](https://msdn.microsoft.com/en-us/library/dn986732(v=vs.85).aspx)`  enumeration. Used to specify the index buffer value to split line strips or triangle strips.
-   `[CD3DX12_PIPELINE_STATE_STREAM_PRIMITIVE_TOPOLOGY](https://msdn.microsoft.com/en-us/library/mt800532(v=vs.85).aspx)`: Wrapper for a  `[D3D12_PRIMITIVE_TOPOLOGY_TYPE](https://msdn.microsoft.com/en-us/library/dn770385(v=vs.85).aspx)`  enumeration. Used to specify the primitive topology type (point, line, triangle or patch). Do not mistake this with the  `[D3D_PRIMITIVE_TOPOLOGY](https://msdn.microsoft.com/en-us/library/ff728726(v=vs.85).aspx)`  enumeration!
-   `[CD3DX12_PIPELINE_STATE_STREAM_VS](https://msdn.microsoft.com/en-us/library/mt800540(v=vs.85).aspx)`: Wrapper for a  `[D3D12_SHADER_BYTECODE](https://msdn.microsoft.com/en-us/library/dn770405(v=vs.85).aspx)`structure. Used to specify the compiled  **Vertex Shader**  (VS).
-   `[CD3DX12_PIPELINE_STATE_STREAM_GS](https://msdn.microsoft.com/en-us/library/mt800527(v=vs.85).aspx)`: Wrapper for a  `[D3D12_SHADER_BYTECODE](https://msdn.microsoft.com/en-us/library/dn770405(v=vs.85).aspx)`structure. Used to specify the compiled  **Geometry Shader**  (GS).
-   `[CD3DX12_PIPELINE_STATE_STREAM_STREAM_OUTPUT](https://msdn.microsoft.com/en-us/library/mt800539(v=vs.85).aspx)`: Wrapper for a  `[D3D12_STREAM_OUTPUT_DESC](https://msdn.microsoft.com/en-us/library/dn770410(v=vs.85).aspx)`  structure. Describes the streaming output buffers for the  **Stream Output**  (SO) stage.
-   `[CD3DX12_PIPELINE_STATE_STREAM_HS](https://msdn.microsoft.com/en-us/library/mt800528(v=vs.85).aspx)`: Wrapper for a  `[D3D12_SHADER_BYTECODE](https://msdn.microsoft.com/en-us/library/dn770405(v=vs.85).aspx)`structure. Used to specify the compiled  **Hull Shader**  (HS).
-   `[CD3DX12_PIPELINE_STATE_STREAM_DS](https://msdn.microsoft.com/en-us/library/mt800525(v=vs.85).aspx)`: Wrapper for a  `[D3D12_SHADER_BYTECODE](https://msdn.microsoft.com/en-us/library/dn770405(v=vs.85).aspx)`structure. Used to specify the compiled  **Domain Shader**  (DS).
-   `[CD3DX12_PIPELINE_STATE_STREAM_PS](https://msdn.microsoft.com/en-us/library/mt800533(v=vs.85).aspx)`: Wrapper for a  `[D3D12_SHADER_BYTECODE](https://msdn.microsoft.com/en-us/library/dn770405(v=vs.85).aspx)`structure. Used to specify the compiled  **Pixel Shader**  (PS).
-   `[CD3DX12_PIPELINE_STATE_STREAM_CS](https://msdn.microsoft.com/en-us/library/mt808296(v=vs.85).aspx)`: Wrapper for a  `[D3D12_SHADER_BYTECODE](https://msdn.microsoft.com/en-us/library/dn770405(v=vs.85).aspx)`structure. Used to specify the compiled  **Compute Shader**  (CS).
-   `[CD3DX12_PIPELINE_STATE_STREAM_BLEND_DESC](https://msdn.microsoft.com/en-us/library/mt808295(v=vs.85).aspx)`: Wraper for a  `[D3D12_BLEND_DESC](https://msdn.microsoft.com/en-us/library/dn770339(v=vs.85).aspx)`structure. Used to describes the blend state.
-   `[CD3DX12_PIPELINE_STATE_STREAM_DEPTH_STENCIL](https://msdn.microsoft.com/en-us/library/mt808297(v=vs.85).aspx)`Wrapper for a  `[D3D12_DEPTH_STENCIL_DESC](https://msdn.microsoft.com/en-us/library/dn770356(v=vs.85).aspx)`  structure. Used to describe the depth-stencil state.
-   `[CD3DX12_PIPELINE_STATE_STREAM_DEPTH_STENCIL1](https://msdn.microsoft.com/en-us/library/mt808298(v=vs.85).aspx)`: Wrapper for a  `[D3D12_DEPTH_STENCIL_DESC1](https://msdn.microsoft.com/en-us/library/mt492556(v=vs.85).aspx)`  structure. Used to describe the depth-stencil state. This version adds the ability to enable depth-bounds testing.
-   `[CD3DX12_PIPELINE_STATE_STREAM_DEPTH_STENCIL_FORMAT](https://msdn.microsoft.com/en-us/library/mt808299(v=vs.85).aspx)`: Wrapper for a  `[DXGI_FORMAT](https://msdn.microsoft.com/en-us/library/bb173059(v=vs.85).aspx)`  enumeration. Describes the format of the depth-stencil buffer.
-   `[CD3DX12_PIPELINE_STATE_STREAM_RASTERIZER](https://msdn.microsoft.com/en-us/library/mt800534(v=vs.85).aspx)`: Wrapper for the  `[D3D12_RASTERIZER_DESC](https://msdn.microsoft.com/en-us/library/dn770387(v=vs.85).aspx)`  structure. Used to describe the  **Rasterizer State**  (RS)
-   `[CD3DX12_PIPELINE_STATE_STREAM_RENDER_TARGET_FORMATS](https://msdn.microsoft.com/en-us/library/mt800535(v=vs.85).aspx)`: Wrapper for the  `[D3D12_RT_FORMAT_ARRAY](https://msdn.microsoft.com/en-us/library/mt492637(v=vs.85).aspx)`  structure which is a wrapper for an array of render target formats.
-   `[CD3DX12_PIPELINE_STATE_STREAM_SAMPLE_DESC](https://msdn.microsoft.com/en-us/library/mt800537(v=vs.85).aspx)`: Wrapper for the  `[DXGI_SAMPLE_DESC](https://msdn.microsoft.com/en-us/library/bb173072(v=vs.85).aspx)`structure. Used to describe the multi-sampling. The render targets must also be created with multisampling enabled.
-   `[CD3DX12_PIPELINE_STATE_STREAM_SAMPLE_MASK](https://msdn.microsoft.com/en-us/library/mt800538(v=vs.85).aspx)`: A  `UINT`  that describes the sample mask for the blend state.
-   `[CD3DX12_PIPELINE_STATE_STREAM_CACHED_PSO](https://msdn.microsoft.com/en-us/library/mt783008(v=vs.85).aspx)`: A wrapper for the  `[D3D12_CACHED_PIPELINE_STATE](https://msdn.microsoft.com/en-us/library/dn914407(v=vs.85).aspx)`  structure. The cached PSO can be used to quickly load the PSO after it has been created. Using cached PSO’s can improve load times.
-   `CD3DX12_PIPELINE_STATE_STREAM_VIEW_INSTANCING`: A wrapper for a  `[D3D12_VIEW_INSTANCING_DESC](https://msdn.microsoft.com/en-us/library/mt844812(v=vs.85).aspx)`  structure. Allows shaders to render to multiple views with a single draw call. Useful for stereo vision or cubemap generation.
...
The  `[D3D12_RT_FORMAT_ARRAY](https://msdn.microsoft.com/en-us/library/mt492637(v=vs.85).aspx)`  structure is used to describe the number of render targtes and the formats of those render targets.
...
To create the pipeline state object, the  `PipelStateStream`  object must be wrapped in a  `[D3D12_PIPELINE_STATE_STREAM_DESC](https://msdn.microsoft.com/en-us/library/mt492579(v=vs.85).aspx)`  structure. This is in-turn passed to the  `[ID3D12Device2::CreatePipelineState](https://msdn.microsoft.com/en-us/library/mt492653(v=vs.85).aspx)`  method to create the final  `[ID3D12PipelineState](https://msdn.microsoft.com/en-us/library/dn788705(v=vs.85).aspx)`object which is what is required for rendering.
...
The  `[ID3D12Device::CreateCommittedResource](https://msdn.microsoft.com/en-us/library/dn899178(v=vs.85).aspx)`  method was previously described in the  `[Tutorial2::UpdateBufferResource](https://www.3dgep.com/learning-directx12-2/#Tutorial2::UpdateBufferResource)`  method. The only difference in this case is the  `[ID3D12Device::CreateCommittedResource](https://msdn.microsoft.com/en-us/library/dn899178(v=vs.85).aspx)`  method is being used to create a texture instead of a buffer.

The  `[D3D12_CLEAR_VALUE](https://msdn.microsoft.com/en-us/library/dn903795(v=vs.85).aspx)`  structure is used to describe the optimized clear color for the depth-stencil buffer. This structure is used for both depth-stencil and color textures.

The  `[CD3DX12_RESOURCE_DESC::Tex2D](https://msdn.microsoft.com/en-us/library/mt186577(v=vs.85).aspx)`  method is a helper function used to create a  `[D3D12_RESOURCE_DESC](https://msdn.microsoft.com/en-us/library/dn903813(v=vs.85).aspx)`  structure. The  `[CD3DX12_RESOURCE_DESC::Tex2D](https://msdn.microsoft.com/en-us/library/mt186577(v=vs.85).aspx)`  method takes the following arguments  [[17]](https://www.3dgep.com/learning-directx12-2/#cite-17):

-   `[DXGI_FORMAT](https://msdn.microsoft.com/en-us/library/bb173059(v=vs.85).aspx) format`: The format of the texture.
...
-   `[D3D12_RESOURCE_FLAGS](https://msdn.microsoft.com/en-us/library/dn986742(v=vs.85).aspx) flags`: A bitwise combination of  `[D3D12_RESOURCE_FLAGS](https://msdn.microsoft.com/en-us/library/dn986742(v=vs.85).aspx)`enumeration values. Since this is a depth-stencil buffer, the  `[D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL](https://msdn.microsoft.com/en-us/library/dn986742(v=vs.85).aspx#D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)`  flag must be specified.
-   `[D3D12_TEXTURE_LAYOUT](https://msdn.microsoft.com/en-us/library/dn770442(v=vs.85).aspx) layout`: Specifies the texture layout options. The default value is  `[D3D12_TEXTURE_LAYOUT_UNKNOWN](https://msdn.microsoft.com/en-us/library/dn770442(v=vs.85).aspx#D3D12_TEXTURE_LAYOUT_UNKNOWN)`  which allows the driver to choose the most optimized layout based on the properties of the texture.
...
The  `[D3D12_DEPTH_STENCIL_VIEW_DESC](https://msdn.microsoft.com/en-us/library/dn770357(v=vs.85).aspx)`  structure is used to describe a depth-stencil resource. The  `[D3D12_DEPTH_STENCIL_VIEW_DESC](https://msdn.microsoft.com/en-us/library/dn770357(v=vs.85).aspx)`  has the following syntax  [[18]](https://www.3dgep.com/learning-directx12-2/#cite-18):
...
-   `[DXGI_FORMAT](https://docs.microsoft.com/en-us/windows/desktop/api/dxgiformat/ne-dxgiformat-dxgi_format) Format`: The depth-stencil format. This should be the same format that was used to create the texture but it could be a different format if the view is for a Shader Resource.
-   `[D3D12_DSV_DIMENSION](https://msdn.microsoft.com/en-us/library/dn770361(v=vs.85).aspx) ViewDimension`: One value from the  `[D3D12_DSV_DIMENSION](https://msdn.microsoft.com/en-us/library/dn770361(v=vs.85).aspx)`enumeration. In this case, it is a 2D texture.
-   `[D3D12_DSV_FLAGS](https://msdn.microsoft.com/en-us/library/dn986727(v=vs.85).aspx) Flags`: A combination of  `[D3D12_DSV_FLAGS](https://msdn.microsoft.com/en-us/library/dn986727(v=vs.85).aspx)`  enumeration constants that are combined by using a bitwise OR operation. The resulting value specifies whether the texture is read only. Pass 0 to specify that it isn’t read only; otherwise, pass one or more of the members of the  `[D3D12_DSV_FLAGS](https://msdn.microsoft.com/en-us/library/dn986727(v=vs.85).aspx)`  enumerated type.
-   One of the following is specified depending on the value of the  `ViewDimension`member:
    -   `[D3D12_TEX1D_DSV](https://msdn.microsoft.com/en-us/library/dn770418(v=vs.85).aspx) Texture1D`: A  `[D3D12_TEX1D_DSV](https://msdn.microsoft.com/en-us/library/dn770418(v=vs.85).aspx)`  structure that specifies a 1D texture subresource.
    -   `[D3D12_TEX1D_ARRAY_DSV](https://msdn.microsoft.com/en-us/library/dn770414(v=vs.85).aspx) Texture1DArray`: A  `[D3D12_TEX1D_ARRAY_DSV](https://msdn.microsoft.com/en-us/library/dn770414(v=vs.85).aspx)`  structure that specifies an array of 1D texture subresources.
    -   `[D3D12_TEX2D_DSV](https://msdn.microsoft.com/en-us/library/dn770432(v=vs.85).aspx) Texture2D`: A  `[D3D12_TEX2D_DSV](https://msdn.microsoft.com/en-us/library/dn770432(v=vs.85).aspx)`  structure that specifies a 2D texture subresource.
    -   `[D3D12_TEX2D_ARRAY_DSV](https://msdn.microsoft.com/en-us/library/dn770428(v=vs.85).aspx) Texture2DArray`: A  `[D3D12_TEX2D_ARRAY_DSV](https://msdn.microsoft.com/en-us/library/dn770428(v=vs.85).aspx)`  structure that specifies an array of 2D texture subresources.
    -   `[D3D12_TEX2DMS_DSV](https://msdn.microsoft.com/en-us/library/dn770425(v=vs.85).aspx) Texture2DMS`: A  `[D3D12_TEX2DMS_DSV](https://msdn.microsoft.com/en-us/library/dn770425(v=vs.85).aspx)`  structure that specifies a multisampled 2D texture.
    -   `[D3D12_TEX2DMS_ARRAY_DSV](https://msdn.microsoft.com/en-us/library/dn770422(v=vs.85).aspx) Texture2DMSArray`: A  `[D3D12_TEX2DMS_ARRAY_DSV](https://msdn.microsoft.com/en-us/library/dn770422(v=vs.85).aspx)`structure that specifies an array of multisampled 2D textures.

On line 268, the depth-stencil view is created using the  `[ID3D12Device::CreateDepthStencilView](https://msdn.microsoft.com/en-us/library/dn788661(v=vs.85).aspx)`  method. The  `[ID3D12Device::CreateDepthStencilView](https://msdn.microsoft.com/en-us/library/dn788661(v=vs.85).aspx)`  method has the following syntax  [[19]](https://www.3dgep.com/learning-directx12-2/#cite-19):
...
-   `[ID3D12Resource](https://msdn.microsoft.com/en-us/library/dn788709(v=vs.85).aspx) *pResource`: A pointer to the  `[ID3D12Resource](https://msdn.microsoft.com/en-us/library/dn788709(v=vs.85).aspx)`  object that represents the depth stencil buffer.  
...
-   `[D3D12_DEPTH_STENCIL_VIEW_DESC](https://msdn.microsoft.com/en-us/library/dn770357(v=vs.85).aspx) *pDesc`: A pointer to a  `[D3D12_DEPTH_STENCIL_VIEW_DESC](https://msdn.microsoft.com/en-us/library/dn770357(v=vs.85).aspx)`  structure that describes the depth-stencil view.  
...
-   `[D3D12_CPU_DESCRIPTOR_HANDLE](https://msdn.microsoft.com/en-us/library/dn859369(v=vs.85).aspx) DestDescriptor`: Describes the CPU descriptor handle that represents the start of the heap that holds the depth-stencil view.
...
The  `m_Viewport`  member variable must match the dimensions of the rederable area of the render target. The  `[CD3DX12_VIEWPORT](https://msdn.microsoft.com/en-us/library/mt779026(v=vs.85).aspx)`  structure provides a convenient method to initialize the viewport.
...
The  `m_ModelMatrix`  is used to store a rotation matrix that rotates the cube about an axis. The DirectX Math library contains several functions that can be used to compute various types of matrices. A rotation matrix can be computed from an angle and a axis of rotation using the  `[XMMatrixRotationAxis](https://msdn.microsoft.com/en-us/library/microsoft.directx_sdk.matrix.xmmatrixrotationaxis(v=vs.85).aspx)`  function.
...
A left-handed view matrix can be computed using the  `[XMMatrixLookAtLH](https://msdn.microsoft.com/en-us/library/microsoft.directx_sdk.matrix.xmmatrixlookatlh(v=vs.85).aspx)`  function from the position of the camera (`eyePosition`), a point to look at (`focusPoint`) and a local up vector (`upDirection`).
...
The  `[CD3DX12_RESOURCE_BARRIER](https://msdn.microsoft.com/en-us/library/mt604704(v=vs.85).aspx)`  structure is a helper structure to enable easy initialization of a  `[D3D12_RESOURCE_BARRIER](https://msdn.microsoft.com/en-us/library/dn986740(v=vs.85).aspx)`  structure.

The  `[ID3D12GraphicsCommandList::ResourceBarrier](https://msdn.microsoft.com/en-us/library/dn903898(v=vs.85).aspx)`  method is used to exectue the resource barrier on the command list. This method can accept an array of resource barriers but for this simple example, one barrier is pushed to the command list at a time.
...
The  `ClearRTV`  method just forwards the function arguments to the more verbosely named  `[ID3D12GraphicsCommandList::ClearRenderTargetView](https://msdn.microsoft.com/en-us/library/dn903842(v=vs.85).aspx)`  method. That method was also described in the  [previous lesson](https://www.3dgep.com/learning-directx12-1/)  and is not described in detail here.
...
The  `[ID3D12GraphicsCommandList::ClearDepthStencilView](https://msdn.microsoft.com/en-us/library/dn903840(v=vs.85).aspx)`  method is used to clear the depth-stencil view specified by the  `dsv`  parameter.
...
The first thing the  `OnRender`  method needs to do is retrieve the  `CommandQueue`  from the  `Application`  class. In the  `[LoadContent](https://www.3dgep.com/learning-directx12-2/#Tutorial2::LoadContent)`  method, a copy queue (`[D3D12_COMMAND_LIST_TYPE_COPY](https://msdn.microsoft.com/en-us/library/dn770348(v=vs.85).aspx#D3D12_COMMAND_LIST_TYPE_COPY)`) was used since only commands to copy resources to the GPU were used. In this case, the  `[D3D12_COMMAND_LIST_TYPE_DIRECT](https://msdn.microsoft.com/en-us/library/dn770348(v=vs.85).aspx#D3D12_COMMAND_LIST_TYPE_DIRECT)`  queue is required to perform the draw command.
...
The  **Depth Stencil View**  (DSV) is retrieved from the depth-stencil descriptor heap. Since there is only a single depth-stencil buffer, the first (and only) handle in the descriptor heap is retrieved using the  `[ID3D12DescriptorHeap::GetCPUDescriptorHandleForHeapStart](https://msdn.microsoft.com/en-us/library/dn899174(v=vs.85).aspx)`  method.

**Exercise**  
Try to reason about why the  **swap chain**  requires multiple back buffers (at least 2) but only a single  **depth-stencil**  buffer is needed for correct rendering. Does the  **depth-stencil**  buffer ever need to be in both a  _read_  and  _write_  state at the same time?  
...
Before clearing, the back buffer resource needs to be transitioned from the  `[D3D12_RESOURCE_STATE_PRESENT](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx#D3D12_RESOURCE_STATE_PRESENT)`  state to the  `[D3D12_RESOURCE_STATE_RENDER_TARGET](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx#D3D12_RESOURCE_STATE_RENDER_TARGET)`  state using the  `[TransitionResource](https://www.3dgep.com/learning-directx12-2/#Tutorial2::TransitionResource)`  method described earlier.

The color buffer is cleared using the  `[ClearRTV](https://www.3dgep.com/learning-directx12-2/#Tutorial2::ClearRTV)`  method and the depth-stencil buffer is cleared using the  `[ClearDepth](https://www.3dgep.com/learning-directx12-2/#Tutorial2::ClearDepth)`  method.
...
The  `[ID3D12GraphicsCommandList::SetPipelineState](https://msdn.microsoft.com/en-us/library/dn903918(v=vs.85).aspx)`  method is used to bind the PSO to the rendering pipeline. This single function call is all that is required to set the various programmable shader stages, input layout, depth-stencil state, and blend state (to name a few things that are defined in the PSO).

Despite already setting the root signature on the PSO during PSO creation (see  [Pipeline State Object](https://www.3dgep.com/learning-directx12-2/#Pipeline_State_Object_1)), the root signature must be explicitly set on the command list using the  `[ID3D12GraphicsCommandList::SetGraphicsRootSignature](https://msdn.microsoft.com/en-us/library/dn903914(v=vs.85).aspx)`  method before binding any resources (CBV, UAV, SRV, or Sampler descriptors). Not explicitly setting the root signature on the command list will result in a runtime error while trying to bind resources.
...
The  **primitive topology type**  (point, line, triangle, or patch) was specified in the PSO but the  **primitive topology**  (for example triangle list, or triangle strip) also needs to be configured using the  `[ID3D12GraphicsCommandList::IASetPrimitiveTopology](https://msdn.microsoft.com/en-us/library/dn903885(v=vs.85).aspx)`  method.

The vertex buffer is bound to the Input Assembler using the  `[ID3D12GraphicsCommandList::IASetVertexBuffers](https://msdn.microsoft.com/en-us/library/dn986883(v=vs.85).aspx)`  method. This method takes the following parameters  [[20]](https://www.3dgep.com/learning-directx12-2/#cite-20):
...
-   `[D3D12_VERTEX_BUFFER_VIEW](https://msdn.microsoft.com/en-us/library/dn903819(v=vs.85).aspx)* pViews`: Specifies the vertex buffer views in an array of  `[D3D12_VERTEX_BUFFER_VIEW](https://msdn.microsoft.com/en-us/library/dn903819(v=vs.85).aspx)`  structures.

The index buffer is bound to the Input Assembler using the  `[ID3D12GraphicsCommandList::IASetIndexBuffer](https://msdn.microsoft.com/en-us/library/dn986882(v=vs.85).aspx)`  method. Only a single index buffer can be bound to the Input Assembler at at time.
...
The viewport is specified using the  `[ID3D12GraphicsCommandList::RSSetViewports](https://msdn.microsoft.com/en-us/library/dn903900(v=vs.85).aspx)`method. This method takes the following parameters  [[21]](https://www.3dgep.com/learning-directx12-2/#cite-21):

-   `UINT NumViewports`: The number of viewports to bind. The range of valid values is (0 to  `[D3D12_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE](https://msdn.microsoft.com/en-us/library/dn903792(v=vs.85).aspx)`). Currently, the maximum number of viewports that can be bound is 16.
-   `[D3D12_VIEWPORT](https://msdn.microsoft.com/en-us/library/dn770454(v=vs.85).aspx)* pViewports`: An array of  `[D3D12_VIEWPORT](https://msdn.microsoft.com/en-us/library/dn770454(v=vs.85).aspx)`  structures to bind to the device.

The viewport to render to is determined by the  `[SV_ViewportArrayIndex](https://msdn.microsoft.com/en-us/library/bb509647(v=vs.85).aspx#System_Value)`  semantic output by the geometry shader. If the geometry shader does not specify the semantic, the first viewport in the array is used.

Since DirectX 12, the scissor rectangle must be explicitly specified using the  `[ID3D12GraphicsCommandList::RSSetScissorRects](https://msdn.microsoft.com/en-us/library/dn903899(v=vs.85).aspx)`  method. Not specifying a scissor rectangle before rendering could result in a empty screen (only the clear color is visible). This method’s paramereters are similar to that of the  `[ID3D12GraphicsCommandList::RSSetViewports](https://msdn.microsoft.com/en-us/library/dn903900(v=vs.85).aspx)`  method.
...
Render targets are bound to the Output Merger stage of the rendering pipeline using the  `[ID3D12GraphicsCommandList::OMSetRenderTargets](https://msdn.microsoft.com/en-us/library/dn986884(v=vs.85).aspx)`  method. This method has the following syntax  [[22]](https://www.3dgep.com/learning-directx12-2/#cite-22):
...
-   `[D3D12_CPU_DESCRIPTOR_HANDLE](https://msdn.microsoft.com/en-us/library/dn859369(v=vs.85).aspx)* pRenderTargetDescriptors`: Specifies an array of  `[D3D12_CPU_DESCRIPTOR_HANDLE](https://msdn.microsoft.com/en-us/library/dn859369(v=vs.85).aspx)`  structures that describe the CPU descriptor handles that represents the start of the heap of render target descriptors.
...
-   `[D3D12_CPU_DESCRIPTOR_HANDLE](https://msdn.microsoft.com/en-us/library/dn859369(v=vs.85).aspx)* pDepthStencilDescriptor`: A pointer to a  `[D3D12_CPU_DESCRIPTOR_HANDLE](https://msdn.microsoft.com/en-us/library/dn859369(v=vs.85).aspx)`  structure that describes the descriptor handle that represents the start of the heap that holds the depth-stencil descriptor.
...
The model, view and projection matrices were update in the  `[OnUpdate](https://www.3dgep.com/learning-directx12-2/#Tutorial2::OnUpdate)`  method shown previously. The three matrices are concatenated together into a single transformation matrix using matrix multiply and passed to the constant buffer in the vertex shader as a set of 32-bit constants. The  `[ID3D12GraphicsCommandList::SetGraphicsRoot32BitConstants](https://msdn.microsoft.com/en-us/library/dn903910(v=vs.85).aspx)`  method is used to upload the matrix to the GPU. This method has the following syntax  [[23]](https://www.3dgep.com/learning-directx12-2/#cite-23):
...
The geometry is drawn using the  `[ID3D12GraphicsCommandList::DrawIndexedInstanced](https://msdn.microsoft.com/en-us/library/dn903874(v=vs.85).aspx)`method. This method has the following syntax  [[24]](https://www.3dgep.com/learning-directx12-2/#cite-24):
...
Before presenting, the back buffer resource needs to be transitioned back to the  `[D3D12_RESOURCE_STATE_PRESENT](https://msdn.microsoft.com/en-us/library/dn986744(v=vs.85).aspx#D3D12_RESOURCE_STATE_PRESENT)`  state.

On line 401, the command list is executed on the command queue. The  `[CommandQueue::ExecuteCommandList](https://www.3dgep.com/learning-directx12-2/#CommandQueue::ExecuteCommandList)`  method returns the fence value to wait for which is saved in the  `m_FenceValues`  array for the next frame.
...
[![Tutorial 2](https://www.3dgep.com/wp-content/uploads/2018/03/Tutorial2-Screenshot.png)](https://www.3dgep.com/wp-content/uploads/2018/03/Tutorial2-Screenshot.png)
...
The  `WheelDelta`  contains the normalized rotation of the mouse wheel. This value is used to increase or decrease the field of view of the camera. To prevent weird distortion of the camera, the field of view should not be allowed to become too small or too large. The field of view is clamped to the range  {12∘⋯90∘}{12∘⋯90∘}
...
[![](https://www.3dgep.com/wp-content/uploads/2011/05/zip1-150x150.png)Source Code.zip](https://github.com/jpvanoosten/LearningDirectX12/archive/v0.0.2.zip)
...
1.  ![](https://secure.gravatar.com/avatar/20c40284c40fd9f990201bf1216398eb?s=68&d=mm&r=g)[Andrew Malcolm](https://www.waikato.ac.nz/)  on  [April 3, 2018 at 2:55 am](https://www.3dgep.com/learning-directx12-2/#comment-59757)  said:
    ...
    [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-59757)
    
2.  ![](https://secure.gravatar.com/avatar/c399ac8bdb60e32a4c902b1dd825e3c8?s=68&d=mm&r=g)Maxim  on  [April 7, 2018 at 7:55 pm](https://www.3dgep.com/learning-directx12-2/#comment-59808)  said:
    
    This is very well explained, and can be understand waiting for more and hungry for knowledge  ![🙂](https://s.w.org/images/core/emoji/11.2.0/svg/1f642.svg)
    ...
    [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-59808)
    
    -   ![](https://secure.gravatar.com/avatar/00f2d6dcb1d4602c73bb571194789ab0?s=39&d=mm&r=g)[Jeremiah van Oosten](https://3dgep.com/)on  [April 9, 2018 at 10:20 am](https://www.3dgep.com/learning-directx12-2/#comment-59827)  said:
        ...
        [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-59827)
        
3.  ![](https://secure.gravatar.com/avatar/fc9688c1160d8c63a0858d1554cdf129?s=68&d=mm&r=g)[Mateusz](https://astralcode.blogspot.com/)  on  [July 8, 2018 at 12:49 pm](https://www.3dgep.com/learning-directx12-2/#comment-61490)  said:
    
    Thank you! The amount of new things to learn in DX12 is insane. It’s hard for me to find good resources for this API.  
    I am definitely waiting for more DX12 lessons  ![🙂](https://s.w.org/images/core/emoji/11.2.0/svg/1f642.svg)
    
    [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-61490)
    
4.  ![](https://secure.gravatar.com/avatar/9a544672618042526a8cc7b01eab0475?s=68&d=mm&r=g)Mathieu D.  on  [July 11, 2018 at 10:12 am](https://www.3dgep.com/learning-directx12-2/#comment-61579)  said:
    ...
    [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-61579)
    
    -   ![](https://secure.gravatar.com/avatar/00f2d6dcb1d4602c73bb571194789ab0?s=39&d=mm&r=g)[Jeremiah van Oosten](https://www.3dgep.com/)on  [August 12, 2018 at 11:16 am](https://www.3dgep.com/learning-directx12-2/#comment-62263)  said:
        
        Part 3 is in progress. My summer vacation has delayed the progress of writing part 3!
        
        [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-62263)
        
        -   ![](https://secure.gravatar.com/avatar/fc9688c1160d8c63a0858d1554cdf129?s=39&d=mm&r=g)[Mateusz](https://astralcode.blogspot.com/)on  [October 27, 2018 at 3:00 pm](https://www.3dgep.com/learning-directx12-2/#comment-64269)  said:
            
            Hi,  
            Is there any info about upcoming parts of this tutorial?
            
            [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-64269)
            
            -   ![](https://secure.gravatar.com/avatar/00f2d6dcb1d4602c73bb571194789ab0?s=39&d=mm&r=g)[Jeremiah van Oosten](https://www.3dgep.com/)on  [November 22, 2018 at 12:47 pm](https://www.3dgep.com/learning-directx12-2/#comment-65488)  said:
                ...
                [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-65488)
                
5.  ![](https://secure.gravatar.com/avatar/539d6928f70ecb5c9615e31bc37e7570?s=68&d=mm&r=g)607  on  [August 13, 2018 at 5:11 am](https://www.3dgep.com/learning-directx12-2/#comment-62300)  said:
    
    The article is great , from which I benefit. I hope you can post the part 3 earily.
    
    [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-62300)
    
6.  ![](https://secure.gravatar.com/avatar/87ada0308ae09e704633a8cfda5900d5?s=68&d=mm&r=g)James Webb  on  [August 18, 2018 at 5:18 pm](https://www.3dgep.com/learning-directx12-2/#comment-62409)  said:
    ...
    [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-62409)
    
    -   ![](https://secure.gravatar.com/avatar/00f2d6dcb1d4602c73bb571194789ab0?s=39&d=mm&r=g)[Jeremiah van Oosten](https://www.3dgep.com/)on  [November 7, 2018 at 7:35 pm](https://www.3dgep.com/learning-directx12-2/#comment-64886)  said:
        
        James,
        
        See the reply below  [in this comment comment](https://www.3dgep.com/learning-directx12-2/#comment-64885).
        
        [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-64886)
        
7.  ![](https://secure.gravatar.com/avatar/87ada0308ae09e704633a8cfda5900d5?s=68&d=mm&r=g)James Webb  on  [August 20, 2018 at 9:38 pm](https://www.3dgep.com/learning-directx12-2/#comment-62450)  said:
    ...
    [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-62450)
    
    -   ![](https://secure.gravatar.com/avatar/00f2d6dcb1d4602c73bb571194789ab0?s=39&d=mm&r=g)[Jeremiah van Oosten](https://www.3dgep.com/)on  [November 7, 2018 at 7:32 pm](https://www.3dgep.com/learning-directx12-2/#comment-64885)  said:
        
        James,
        
        Good catch! The vertex and index buffers do not need an explicit resource barrier because resources used on a copy queue (or buffer resources used on any queue type) will automatically decay to the  `D3D12_RESOURCE_STATE_COMMON`  resource state when the GPU finishes execution of  `[ID3D12CommandQueue::ExecuteCommandLists](https://docs.microsoft.com/en-us/windows/desktop/api/d3d12/nf-d3d12-id3d12commandqueue-executecommandlists)`. Resource state decay to the  `COMMON`  state is free and no additional resource barrier is required.
        ...
        This is called  _implicit state transitions_  and is further explained here:  [Implicit State Transitions](https://docs.microsoft.com/en-us/windows/desktop/direct3d12/using-resource-barriers-to-synchronize-resource-states-in-direct3d-12#implicit-state-transitions).
        
        [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-64885)
        
8.  ![](https://secure.gravatar.com/avatar/073701c7483c1d98457f3269401851af?s=68&d=mm&r=g)Artem  on  [December 10, 2018 at 11:06 pm](https://www.3dgep.com/learning-directx12-2/#comment-65961)  said:
    ...
    [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-65961)
    
    -   ![](https://secure.gravatar.com/avatar/00f2d6dcb1d4602c73bb571194789ab0?s=39&d=mm&r=g)[Jeremiah van Oosten](https://www.3dgep.com/)on  [December 11, 2018 at 2:32 pm](https://www.3dgep.com/learning-directx12-2/#comment-65969)  said:
        ...
        [Reply  ↓](https://www.3dgep.com/learning-directx12-2/#comment-65969)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjk5NzU1MDAxXX0=
-->