...
Текстурные координаты обычно выражаются в ** нормализованном пространстве текстуры **.Texture coordinates are usually expressed in  **normalized texture space**. This means that the top-left texture element (texel) is at  **(0, 0)**  and the bottom-right texel is at  **(1, 1)**.
Координаты текстуры обычно выражаются в ** нормализованном пространстве текстуры **. Это означает, что верхний левый элемент текстуры (texel) имеет значение ** (0, 0) **, а нижний правый texel имеет значение ** (1, 1) **.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg2MDEzOTgzOCwyNjUxODU2ODddfQ==
-->