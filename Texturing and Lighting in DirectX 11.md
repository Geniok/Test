...
Настройки фильтра состояния сэмплера текстуры определяют, как выбранный тексель смешивается с соседними текселями для получения окончательного цвета текселя. Обычно существует три типа фильтрации; **точечная** , **линейная** и **анизотропная**. Точечная фильтрация просто переносит ближайший тексель к выбранному субтекселю. Линейная фильтрация будет применять билинейную смесь между ближайшими текселями к выбранному субтекстелю, используя расстояние до центра текселя в качестве веса, используемого для смешения текселей для получения конечного текселя. Анизотропная фильтрация применяется при выборке поверхностей, которые наклонены (под углом угол) к зрителю. Анизотропная фильтрация обычно используется при выборке текстур, которые применяются к грунтовой плоскости или местности
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwODg4ODg2NjUsLTE5MTM2NDE2MjgsLT
gwNTg1NDkyM119
-->