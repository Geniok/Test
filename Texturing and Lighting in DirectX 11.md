...
Then, <img src="https://latex.codecogs.com/gif.latex/?\begin{array}{rcl}\mathbf{H}&=&\text{normalize}(\mathbf{L}+\mathbf{V})\\specular&=&(\mathbf{N}\cdot\mathbf{H})^{\alpha}*\mathbf{L}_s*k_s\end{array})"/>

When using the Blinn-Phong lighting model and if the light is a directional light (for example to simulate the sun) and we compute lighting in view space, then both  <img src="https://latex.codecogs.com/gif.latex/?\mathbf{L}"/>  and  <img src="https://latex.codecogs.com/gif.latex/?\mathbf{V}"/>  are constants, therefore the half-angle vector (<img src="https://latex.codecogs.com/gif.latex/?\mathbf{H}"/>) between  <img src="https://latex.codecogs.com/gif.latex/?\mathbf{L}"/>  and  <img src="https://latex.codecogs.com/gif.latex/?\mathbf{V}"/>  is also constant and can be passed as a uniform variable to our shader (one uniform variable per directional light in the scene). This eliminates the need to compute  <img src="https://latex.codecogs.com/gif.latex/?\mathbf{L}"/>,  <img src="https://latex.codecogs.com/gif.latex/?\mathbf{V}"/>  and  <img src="https://latex.codecogs.com/gif.latex/?\mathbf{H}"/>  in the shader which saves at least three normalize function invocations per pixel (or vertex if performing per-vertex lighting). However, for positional lights (point and spot lights) we still need to at least compute both the <img src="https://latex.codecogs.com/gif.latex/?\mathbf{L}"/>  and  <img src="https://latex.codecogs.com/gif.latex/?\mathbf{N}"/>![](https://www.3dgep.com/texturing-lighting-directx-11/?\mathbf{H})  vectors which reduces the advantages of this optimization.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNDI1MzczNTMsNDM0NTg2NzczXX0=
-->